...
static NSString *notificationsQuery = @"SELECT ID, Name, isFeedbackAllowed__c, Case_Process_Name__c, Case_Status__c, isMessageRead__c, Is_Push_Notification_Allowed__c, Notification_Message__c, Prior_Value__c, Read_Date_and_Time__c, Compiled_Message__c, CreatedDate, Case__r.Id, Case__r.CaseNumber, Case__r.Case_Rating_Score__c, Case__r.Web_Form__c, Case__r.Status, Case__r.CreatedDate FROM Notification_Management__c WHERE Case__r.AccountId = '%@' AND Is_Push_Notification_Allowed__c = TRUE ORDER BY CreatedDate DESC LIMIT %d OFFSET %d";
...
static NSString *contractsQuery = @"SELECT Id, Name, Contract_Number__c, Contract_Type__c, Activated_Date__c, Total_Rent_Price__c, Contract_Duration__c, IS_BC_Contract__c, Rent_Start_date__c, Contract_Duration_Year_Month__c, Contract_Start_Date__c, Contract_Expiry_Date__c, Status__c FROM Contract_DWC__c WHERE Status__c = 'Active' AND Tenant__c = '%@' ORDER BY CreatedDate LIMIT %%d OFFSET %%d";
