...
static NSString *contractorsQuery = @"SELECT Id, Name, Personal_Photo__c, Card_Number__c, Status__c, Sponsor__c, Card_Type__c, Salutation__c, Card_Expiry_Date__c, Card_Issue_Date__c, Full_Name__c, Designation__c, Duration__c, Passport_Number__c, RecordType.Id, RecordType.Name, RecordType.DeveloperName, Nationality__r.Id, Nationality__r.Name FROM Card_Management__c WHERE Account__c = '%@' and Status__c NOT IN ('Renewed') AND Card_Type__c NOT IN ('Employee Card','Student Card') AND Status__c LIKE '%%@' ORDER BY Card_Expiry_Date__c DESC LIMIT %%d OFFSET %%d";
...
static NSString *caseReviewQuery = @"SELECT CaseNumber, CreatedDate, Status, Type, isCourierRequired__c, Employee_Ref__r.Id, Employee_Ref__r.Name, Service_Requested__r.Total_Amount__c, Application_Rejection_Reason__c, (SELECT ID, Name, Status__c, Amount__c FROM Invoices__r), (SELECT ID, Name, Status__c, Amount__c FROM Invoices1__r)";
...
static NSString *companyShareholdersQuery = @"SELECT Id, No_of_Shares__c, Ownership_of_Share__c, Shareholder__r.Id, Shareholder__r.Name, Shareholder__r.Nationality__c, Shareholder__r.Personal_Photo__pc, Shareholder__r.Current_Passport__r.Id, Shareholder__r.Current_Passport__r.Name, Shareholder__r.Current_Passport__r.Passport_Expiry_Date__c, Shareholder__r.Current_Passport__r.Passport_Issue_Date__c, Shareholder__r.Current_Passport__r.Passport_Type__c, Shareholder__r.Current_Passport__r.Passport_Place_of_Issue__c, Shareholder_Status__c, Ownership_End_Date__c, Ownership_Start_Date__c FROM Share_Ownership__c WHERE Company__c = '%@' ORDER BY Shareholder__r.Name LIMIT %%d OFFSET %%d";

static NSString *companyManagersQuery = @"SELECT Id, Manager__r.Id, Manager__r.Name, Manager_Status__c, Manager__r.Personal_Photo__pc, Manager__r.Nationality__c, Manager__r.Current_Passport__r.Id, Manager__r.Current_Passport__r.Name, Manager__r.Current_Passport__r.Passport_Expiry_Date__c, Manager__r.Current_Passport__r.Passport_Issue_Date__c, Manager__r.Current_Passport__r.Passport_Type__c, Manager__r.Current_Passport__r.Passport_Place_of_Issue__c, Manager_Start_Date__c, Manager_End_Date__c, Role__c, Status__c FROM Management_Member__c WHERE Company__c = '%@' ORDER BY Manager__r.Name LIMIT %%d OFFSET %%d";

static NSString *companyDirectorsQuery = @"SELECT Id, Roles__c, Director_Status__c, Directorship_End_Date__c, Directorship_Start_Date__c, Director__r.Id, Director__r.Name, Director__r.Personal_Photo__pc ,Director__r.Nationality__c, Director__r.Current_Passport__r.Id, Director__r.Current_Passport__r.Name, Director__r.Current_Passport__r.Passport_Expiry_Date__c, Director__r.Current_Passport__r.Passport_Issue_Date__c, Director__r.Current_Passport__r.Passport_Type__c, Director__r.Current_Passport__r.Passport_Place_of_Issue__c FROM Directorship__c WHERE Company__c = '%@' ORDER BY Director__r.Name LIMIT %%d OFFSET %%d";

static NSString *companyLegalRepresentativesQuery = @"SELECT Id, Legal_Representative__r.Id, Legal_Representative__r.Name, Legal_Representative__r.Nationality__c, Legal_Representative__r.Personal_Photo__pc, Legal_Representative__r.Current_Passport__r.Id, Legal_Representative__r.Current_Passport__r.Name, Legal_Representative__r.Current_Passport__r.Passport_Expiry_Date__c, Legal_Representative__r.Current_Passport__r.Passport_Issue_Date__c, Legal_Representative__r.Current_Passport__r.Passport_Type__c, Legal_Representative__r.Current_Passport__r.Passport_Place_of_Issue__c, Status__c, Role__c, Legal_Representative_End_Date__c, Legal_Representative_Start_Date__c FROM Legal_Representative__c WHERE Company__c = '%@' ORDER BY Legal_Representative__r.Name LIMIT %%d OFFSET %%d";
...
static NSString *contractsQuery = @"select IS_BC_Contract__c , id , name , Contract_Type__c , Activated_Date__c , Total_Rent_Price__c , Contract_Duration__c , Rent_Start_date__c , Contract_Duration_Year_Month__c , Contract_Start_Date__c ,Contract_Expiry_Date__c, (Select Inventory_Unit__r.Product_Type__r.Name , Product__c, Quantity__c, Total_Price__c, Unit_name__c From Contract_Line_Items__r) from Contract_DWC__C where Status__c = 'Active' and Tenant__c = '%@'";
...
+ (NSString *)freeZonePaymentsQueryWithLimit:(NSInteger)limit offset:(NSInteger)offset datesFilter:(NSString *)datesFilter {
    ...
    if (![datesFilter isEqualToString:@""])
        queryFilter = [NSString stringWithFormat:@"AND %@", datesFilter];
    
    return [NSString stringWithFormat:freeZonePaymentsQuery, [Globals currentAccount].Id, queryFilter, limit, offset];
}
