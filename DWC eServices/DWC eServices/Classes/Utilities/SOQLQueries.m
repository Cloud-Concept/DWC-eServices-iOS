...
static NSString *myRequestsQuery = @"SELECT Id, CaseNumber, Status, Type, Web_Form__c, CreatedDate, RecordType.Id, RecordType.Name, RecordType.DeveloperName, RecordType.SobjectType FROM Case WHERE AccountId = '%@' ORDER BY CreatedDate DESC";
...
static NSString *contractsQuery = @"SELECT Id, Name, Contract_Number__c, Contract_Type__c, Activated_Date__c, Total_Rent_Price__c, Contract_Duration__c, IS_BC_Contract__c, Rent_Start_date__c, Contract_Duration_Year_Month__c, Contract_Start_Date__c, Contract_Expiry_Date__c, Status__c FROM Contract_DWC__c WHERE Status__c = 'Active' AND Tenant__c = '%@'";

static NSString *dwcDocumentsQuery = @"SELECT Id, Name, Template_Name_Link__c, Available_for_Preview__c, Original_can_be_Requested__c, eService_Administration__r.New_Edit_VF_Generator__c, eService_Administration__r.Redirect_Page__c, eService_Administration__r.Id, eService_Administration__r.Related_to_Object__c, eService_Administration__r.Display_Name__c FROM eServices_Document_Checklist__c WHERE eService_Administration__r.Related_to_Object__c = 'Registration' AND eService_Administration__r.RecordType.DeveloperName = 'Service_Request' AND eService_Administration__r.Is_Active__c = true AND eService_Administration__r.Sub_Category__c = 'Registration Services'";
