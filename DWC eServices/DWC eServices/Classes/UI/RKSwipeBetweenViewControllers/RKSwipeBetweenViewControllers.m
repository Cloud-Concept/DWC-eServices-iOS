...
#define HEIGHT 44 //%%% height of the segment
...
-(void)setupSegmentButtons
{
    ...
    for (int i = 0; i<numControllers; i++) {
        ...
        button.backgroundColor = [UIColor colorWithRed:0.95f
                                                 green:0.95f
                                                  blue:0.95f
                                                 alpha:1];//%%% buttoncolors
        
        [button addTarget:self action:@selector(tapSegmentButtonAction:) forControlEvents:UIControlEventTouchUpInside];
        
        [button setTitle:[buttonText objectAtIndex:i] forState:UIControlStateNormal]; //%%%buttontitle
        [button setTitleColor:[UIColor colorWithRed:0.1f
                                              green:0.4f
                                               blue:0.67f
                                              alpha:1.0f]
                     forState:UIControlStateNormal];
        
        [button.titleLabel setFont:[UIFont fontWithName:@"CorisandeLight" size:14]];
        [button setBackgroundImage:[UIImage imageNamed:@"Pagination View Button Frame"] forState:UIControlStateNormal];
    }
...
-(void)setupSelector
{
    ...
    selectionBar.backgroundColor = [UIColor colorWithRed:0.1f
                                                   green:0.4f
                                                    blue:0.67f
                                                   alpha:1.0f]; //%%% sbcolor
...
-(void)refreshButtonViews {
    for(UIView *buttonView in navigationView.subviews) {
        if ([buttonView isKindOfClass:[UIButton class]]) {
            ...
            if(button.tag == currentPageIndex) {
                backgroundColor = [UIColor colorWithRed:0.93f
                                                  green:0.93f
                                                   blue:0.93f
                                                  alpha:1];
                
                titleColor = [UIColor colorWithRed:0.1f
                                             green:0.4f
                                              blue:0.67f
                                             alpha:1.0f];
            } else {
                backgroundColor = [UIColor colorWithRed:0.95f
                                                  green:0.95f
                                                   blue:0.95f
                                                  alpha:1];
                
                titleColor = [UIColor colorWithRed:0.63f
                                             green:0.63f
                                              blue:0.63f
                                             alpha:1.0f];
            }
