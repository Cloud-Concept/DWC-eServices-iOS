...
+ (NSArray *)setupCustomActivityIndicator {
    ...
    for (int i = 1; i <= 20; i++) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"FVCustomAlertView %d",i]];
...
+ (UIView *)contentViewFromType:(FVAlertType)type {
    ...
    switch (type) {
        ...
        case FVAlertTypeDone:
        {
            ...
            content.image = [UIImage imageNamed:@"FVCustomAlertView checkmark"];
        }
            break;
        case FVAlertTypeError:
        {
            content.frame = CGRectMake(0, kInsetValue, kOtherIconsSize, kOtherIconsSize);
            content.image = [UIImage imageNamed:@"FVCustomAlertView cross"];
        }
            break;
        case FVAlertTypeWarning:
        {
            content.frame = CGRectMake(0, kInsetValue, kOtherIconsSize, kOtherIconsSize);
            content.image = [UIImage imageNamed:@"FVCustomAlertView warning"];
        }
            break;
        default:
...
+ (void)hideAlertFromMainWindowWithFading:(BOOL)fading {
    ...
    if (fading) {
        [self fadeOutView:[window viewWithTag:kFinalViewTag] completion:^(BOOL finished) {
            [[window viewWithTag:kFinalViewTag] removeFromSuperview];
        }];
    } else {
        [[window viewWithTag:kFinalViewTag] removeFromSuperview];
    }
